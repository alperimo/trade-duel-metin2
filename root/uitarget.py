...
class TargetBoard(ui.ThinBoard):
	...
	def __OnEsyaDuello(self):
		if player.IsChallengeInstance(self.vid) and gameInfo.DUELLO_DURUM == 0:
			...
			net.SendWhisperPacket(chr.GetNameByVID(self.vid), "##xsbyfatihbab34axsadsahdgsadsadassadjadada##")
			net.SendChatPacket("/pvp %d" % (self.vid))
			gameInfo.DUELLO_YOLLANANLAR.append("#"+str(chr.GetNameByVID(self.vid))+"#")
			gameInfo.DUELLO_VID = self.vid
		elif gameInfo.DUELLO_DURUM == 3: ## duello kabul ##
			net.SendWhisperPacket(chr.GetNameByVID(self.vid), "#xsbyfatihbab34sdajdsadnsadmnasdmnaswmqwmnmdsnaddasawq#")
			gameInfo.DUELLO_VID = self.vid
		elif gameInfo.DUELLO_DURUM == 5: ## duello kabul 2.ci ##
			net.SendWhisperPacket(chr.GetNameByVID(self.vid), "#xsbyfatihbab34-sdajdsadnsadmnasdmnaswmqwmnmdsnaddasawq#")
			gameInfo.DUELLO_VID = self.vid
		elif gameInfo.DUELLO_DURUM == 1: ## eðer karakter duelloda ise ##
			self.Chat("<Düello> : 1'den fazla kiþiyle ayný anda duello yapamassýn.")
